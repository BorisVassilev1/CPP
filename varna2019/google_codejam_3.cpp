...
InfInt GCD(InfInt a, InfInt b) {
    while(true)
    {
        ...
        else{ b %= a; if(b == 0){return a;}}
    }
    return a;
}

std::vector<std::pair<InfInt, int> > primes;
...
int main() {
    ...
    for(int t =0 ; t < T; t ++) {
        ...
        for(int i = 0;i < L; i ++) {
            ...
            std::cin >> curr;
            if(i >= 1) {
                InfInt primeNumberForTheSecondLetter = GCD(last,curr);
                primes.push_back(make_pair(primeNumberForTheSecondLetter,i));
                if(i == 1) primes.push_back(make_pair(last/primeNumberForTheSecondLetter,0));
                if(i )
            }
            prev = curr;
        }
