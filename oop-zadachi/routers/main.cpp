...
class Router {
    private:
        ...
        RouteInfo* search_table(const string& address) {
            for(auto i = routing_table.begin(); i != routing_table.end(); i ++) {
                if((*i)->router_ip == address) {
                    return *i;
                }
            }
            return nullptr;
    ...
    public:
        ...
        void add_router(const Router& router) {
            if(router.ip == "0.0.0.0" || router.ip == "127.0.0.0")
        ...
        int query_route(const string& address, const int hop_count) {
            if(address == ip) return 1;
            if(hop_count == 0) return 0;
            if(search_table(address) != nullptr) return 1;
        ...
        void send_package(const Package& package) { 
            ...
            if(package.get_sender_ip() == "0.0.0.0" ||
                    package.get_sender_ip() == "127.0.0.0" ||
                    package.get_receiver_ip() == "0.0.0.0" ||
                    package.get_receiver_ip() == "127.0.0.0")
                throw InvalidPackageException("Router: Tried sending an instance of 'Package' with invalid sender or receiver IP.");

            if(package.get_receiver_ip() == ip) {
            ...
            for(auto i = routing_table.begin(); i != routing_table.end(); i ++) {
                RouteInfo* info = *i;
                if(package.get_receiver_ip() == info->router_ip) {
        ...
        void print_routing_table() {
            cout << "routing table for router: " << name << endl;
            for(auto i = routing_table.begin(); i != routing_table.end(); i ++) {
                cout << **i << endl;
            }
        }
        ...
        void print_routers() {
            cout << "routing connections for router: " << name << endl;
...
const int Router::max_hop_count = 15;
const int Router::max_table_size = 5;
...
int main() {
    ...
    Router* curr_r;
    while(!in.eof() && input_router(in, curr_r)) {
    ...
    while(!in.eof() && input_package(in, curr_p)) {
